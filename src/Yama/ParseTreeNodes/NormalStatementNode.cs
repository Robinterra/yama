...
namespace Yama.Parser
{
    public class NormalStatementNode : IParseTreeNode, IIndexNode, ICompileNode, IContainer
    {
        ...
        private void CompileMemoryClean(RequestParserTreeCompile request, MethodeCallNode mcn)
        {
            ...
            request.Compiler.AddError($"Possible of memory leak\n\tsolution: {mdn.Deklaration.ReturnValue.Name} yourGarbageVariableName = ...{mdn.Deklaration.Name}(...)\nSave the return value to a variable, so that memory can be cleaned automaticly\n", this);
        }
