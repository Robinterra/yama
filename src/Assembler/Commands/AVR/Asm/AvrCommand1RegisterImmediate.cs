...
namespace Yama.Assembler.Commands.AVR.Asm
{
    ...
    public class AvrCommand1RegisterImmediate: ICommand
    {
        ...
        public AvrCommand1RegisterImmediate(string key, string format, uint id, int size)
        {
            this.Key = key;
            this.Format = format;
            this.CommandId = id;
        ...
        public AvrCommand1RegisterImmediate(AvrCommand1RegisterImmediate t, IParseTreeNode node, List<byte> bytes)
        {
            this.Key = t.Key;
        ...
        public bool Assemble(RequestAssembleCommand request)
        {
            ...
            assembleFormat.Arguments.Add((uint)t.Argument1.Token.Value);

            if (!format.Assemble(assembleFormat)) return false;

            if (request.WithMapper) request.Result.Add(new AvrCommand1RegisterImmediate(this, request.Node, assembleFormat.Result));
